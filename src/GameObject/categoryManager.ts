class CategoryManager {
    ...
    public setCategory(categoryType: string, categoryNum: number) {
        if (this.cacheN2S.has(categoryNum) || this.cacheS2N.has(categoryType)) {
            throw new Error(`${categoryType}:${categoryNum} categoryType or categoryNum had been set`);
        } else {
    ...
    public getCategory(categoryType: string): number {
        ...
        if (!getNum) throw new Error(`${categoryType} this categoryType had not been set`);
        return getNum;
    }
}
